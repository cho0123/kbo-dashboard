...
export function drawShorts4IntroSlide(ctx, w, h, date, logosByTeamKey, firstGame) {
  ...
  const logoBox = 550;
  const awayLogoX = w - 450;
  const awayLogoY = h * 0.18;
  const homeLogoX = -100;
  const homeLogoY = h * 0.52;
  ...
  const matchupY = dateY + dateFontPx + 40;
  let mFontPx = 80;
  ctx.save();
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.font = `700 ${mFontPx}px "${FONT_BODY}", system-ui, sans-serif`;
  while (ctx.measureText(matchupStr).width < w * 0.7 && mFontPx < 200) {
    mFontPx += 2;
    ctx.font = `700 ${mFontPx}px "${FONT_BODY}", system-ui, sans-serif`;
  }
  ctx.fillStyle = "#ffffff";
